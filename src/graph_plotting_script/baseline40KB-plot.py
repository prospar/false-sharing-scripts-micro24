...
PATH_CSV = "/home/prospar/micro-2024-result/result/micro-baseline-40kb/Stats_Avg.csv"
...
def plot_time():
  ...
  plot_data(fs_norm_times, man_fix_norm_times, "Speedup", "baseline40KB-run-time.pdf")
...
def plot_traffic_vol():
  ...
  geo_mean_vol = round(np.power(np.prod(man_fix_norm_traffic), 1.0/len(man_fix_norm_traffic)),PRECISION)
  ...
  plot_data(fs_norm_traffic, man_fix_norm_traffic, "Normalized on-chip traffic \n(bytes)",
            "baseline40KB-traffic-vol.pdf", True)
...
def plot_traffic_count():
  ...
  geo_mean_count = round(np.power(np.prod(man_fix_norm_traffic), 1.0/len(man_fix_norm_traffic)),PRECISION)
  fs_norm_traffic.append(1.0)
  man_fix_norm_traffic.append(geo_mean_count)
  plot_data(fs_norm_traffic, man_fix_norm_traffic, "Normalized on-chip traffic \n(messages)",
            "baseline40KB-traffic-count.pdf")
...
def plot_energy():
  ...
  geo_mean_energy = round(np.power(np.prod(man_fix_norm_energy), 1.0/len(man_fix_norm_energy)),PRECISION)
  fs_norm_energy.append(1.0)
  man_fix_norm_energy.append(geo_mean_energy)
  plot_data(fs_norm_energy, man_fix_norm_energy, "Normalized energy usage", "baseline40KB-energy.pdf")
