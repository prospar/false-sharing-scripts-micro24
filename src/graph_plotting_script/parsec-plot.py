...
PATH_CSV = "/home/vipin/Documents/false-sharing-result/micro-2024-result/result/parsec-final/Stats_Avg.csv"
...
def plot_time():
  ...
  plot_data(fs_norm_times, repair_norm_times, "Speedup", "plot-parsec-runtime.pdf")
...
def plot_traffic_vol():
  ...
  geomean = np.prod(repair_norm_traffic) ** (1.0 / len(repair_norm_traffic))
  repair_norm_traffic.append(round(geomean,PRECISION))
  plot_data(fs_norm_traffic, repair_norm_traffic,  "On-chip traffic \n(bytes)",
            "plot-parsec-traffic-vol.pdf", True)

def plot_traffic_count():
  fs_abs_traffic = []
  # detect_abs_traffic = []ss
  repair_abs_traffic = []
  for bench in BENCH_NAMES:
...
def plot_energy():
  ...
  for bench in BENCH_NAMES:
    ...
    #                           bench_detect_pamenergy + bench_detect_samenergy)
    bench_repair = bench + "-repair"
  ...
  plot_data(fs_norm_energy, repair_norm_energy, "Normalized\n energy usage", "plot-parsec-energy.pdf")
