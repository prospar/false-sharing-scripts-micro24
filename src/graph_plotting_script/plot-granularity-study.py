...
PATH_CSV = "/home/vipin/Documents/false-sharing-result/micro-2024-result/result/granularity-study/Stats_Avg.csv"
...
def plot_time():
  ...
  plot_data(gran2_norm_times, gran4_norm_times, "Speedup", "gran-run-time.pdf")
...
def plot_traffic_vol():
  ...
  for bench in BENCH_NAMES:
    bench_base = bench + "-repair"
    bench_fs_traffic = bench_data[bench_base][KEY_MSG_VOL]
    fs_abs_traffic.append(bench_fs_traffic)
    bench_gran2 = bench + "-gran2"
    bench_gran2_traffic = bench_data[bench_gran2][KEY_MSG_VOL]
    gran2_abs_traffic.append(bench_gran2_traffic)
    # repair protocol
    bench_gran4 = bench + "-gran4"
    bench_gran4_traffic = bench_data[bench_gran4][KEY_MSG_VOL]
    gran4_abs_traffic.append(bench_gran4_traffic)

  fs_norm_traffic = [1 for x in fs_abs_traffic]
  gran2_norm_traffic = [
    round(n / d, PRECISION) for n, d in zip(gran2_abs_traffic, fs_abs_traffic)
  ]
  geo_mean_vol = round(np.power(np.prod(gran2_norm_traffic), 1.0/len(gran2_norm_traffic)),PRECISION)
  fs_norm_traffic.append(1.0)
  gran2_norm_traffic.append(geo_mean_vol)
  gran4_norm_traffic = [
    round(n / d, PRECISION) for n, d in zip(gran4_abs_traffic, fs_abs_traffic)
  ]
  geo_mean_vol = round(np.power(np.prod(gran4_norm_traffic), 1.0/len(gran4_norm_traffic)),PRECISION)
  gran4_norm_traffic.append(geo_mean_vol)
  plot_data(gran2_norm_traffic, gran4_norm_traffic,  "Normalized on-chip traffic \n(bytes)",
            "gran-traffic-vol.pdf", True)
...
def plot_traffic_count():
  ...
  for bench in BENCH_NAMES:
    bench_base = bench+"-repair"
    bench_fs_traffic = bench_data[bench_base][KEY_MSG_COUNT]
    fs_abs_traffic.append(bench_fs_traffic)
    bench_gran2 = bench + "-gran2"
    bench_gran2_traffic = bench_data[bench_gran2][KEY_MSG_COUNT]
    gran2_abs_traffic.append(bench_gran2_traffic)
    # repair protocol
    bench_gran4 = bench + "-gran4"
    bench_gran4_traffic = bench_data[bench_gran4][KEY_MSG_COUNT]
    gran4_abs_traffic.append(bench_gran4_traffic)

  fs_norm_traffic = [1 for x in fs_abs_traffic]
  gran2_norm_traffic = [
    round(d / n, PRECISION) for n, d in zip(gran2_abs_traffic, fs_abs_traffic)
  ]
  geo_mean_count = round(np.power(np.prod(gran2_norm_traffic), 1.0/len(gran2_norm_traffic)),PRECISION)
  fs_norm_traffic.append(1.0)
  gran2_norm_traffic.append(geo_mean_count)
  gran4_norm_traffic = [
    round(n / d, PRECISION) for n, d in zip(gran4_abs_traffic, fs_abs_traffic)
  ]
  geo_mean_count = round(np.power(np.prod(gran4_norm_traffic), 1.0/len(gran4_norm_traffic)),PRECISION)
  gran4_norm_traffic.append(geo_mean_count)
  plot_data(gran2_norm_traffic, gran4_norm_traffic, "Normalized on-chip traffic \n(messages)",
            "gran-traffic.pdf")
...
def plot_energy():
  ...
  for bench in BENCH_NAMES:
    bench_base = bench+"-repair"
    bench_fs_l1denergy = bench_data[bench_base][KEY_ENERGY_L1D]
    bench_fs_llcenergy = bench_data[bench_base][KEY_ENERGY_LLC]
    bench_fs_pamenergy = bench_data[bench_base][KEY_ENERGY_PAM]
    bench_fs_samenergy = bench_data[bench_base][KEY_ENERGY_SAM]
    bench_fs_cpu2l1denergy = bench_data[bench_base][KEY_ENERGY_CPU2L1D]
    bench_fs_fillenergy = bench_data[bench_base][KEY_ENERGY_FILL]
    bench_fs_staticleakage = bench_data[bench_base][KEY_TOTAL_LEAKAGE]
    ...
    bench_gran2 = bench + "-gran2"
    bench_gran2_l1denergy = bench_data[bench_gran2][KEY_ENERGY_L1D]
    bench_gran2_llcenergy = bench_data[bench_gran2][KEY_ENERGY_LLC]
    bench_gran2_pamenergy = bench_data[bench_gran2][KEY_ENERGY_PAM]
    bench_gran2_samenergy = bench_data[bench_gran2][KEY_ENERGY_SAM]
    bench_gran2_cpu2l1denergy = bench_data[bench_gran2][KEY_ENERGY_CPU2L1D]
    bench_gran2_fillenergy = bench_data[bench_gran2][KEY_ENERGY_FILL]
    bench_gran2_staticleakage = bench_data[bench_gran2][KEY_TOTAL_LEAKAGE]
    # gran2_abs_energy.append(bench_gran2_l1denergy + bench_gran2_llcenergy +
    gran2_abs_energy.append(bench_gran2_pamenergy + bench_gran2_samenergy
                             + bench_gran2_cpu2l1denergy + bench_gran2_fillenergy
                              + bench_gran2_staticleakage)
    bench_gran4 = bench + "-gran4"
    bench_gran4_l1denergy = bench_data[bench_gran4][KEY_ENERGY_L1D]
    bench_gran4_llcenergy = bench_data[bench_gran4][KEY_ENERGY_LLC]
    bench_gran4_pamenergy = bench_data[bench_gran4][KEY_ENERGY_PAM]
    bench_gran4_samenergy = bench_data[bench_gran4][KEY_ENERGY_SAM]
    bench_gran4_cpu2l1denergy = bench_data[bench_gran4][KEY_ENERGY_CPU2L1D]
    bench_gran4_fillenergy = bench_data[bench_gran4][KEY_ENERGY_FILL]
    bench_gran4_staticleakage = bench_data[bench_gran4][KEY_TOTAL_LEAKAGE]
    # gran4_abs_energy.append(bench_gran4_l1denergy + bench_gran4_llcenergy +
    gran4_abs_energy.append(bench_gran4_pamenergy + bench_gran4_samenergy
                             + bench_gran4_cpu2l1denergy + bench_gran4_fillenergy
                             + bench_gran4_staticleakage)

  fs_norm_energy = [1 for x in fs_abs_energy]
  gran2_norm_energy = [round(n / d, PRECISION) for n, d in zip(gran2_abs_energy, fs_abs_energy)]
  geo_mean_energy = round(np.power(np.prod(gran2_norm_energy), 1.0/len(gran2_norm_energy)),PRECISION)
  gran2_norm_energy.append(geo_mean_energy)

  gran4_norm_energy = [round(n / d, PRECISION) for n, d in zip(gran4_abs_energy, fs_abs_energy)]
  geo_mean_energy = round(np.power(np.prod(gran4_norm_energy), 1.0/len(gran4_norm_energy)),PRECISION)
  gran4_norm_energy.append(geo_mean_energy)
  fs_norm_energy.append(1.0)

  plot_data(gran2_norm_energy, gran4_norm_energy, "Normalized energy usage", "gran-energy.pdf")
